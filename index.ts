...
/**
 * Custom entry type marking that MCP was enabled for this session via
 * /enable-mcp. Persisted in the session file so activation survives a
 * reload (e.g. /mcp setup triggering ctx.reload()).
 */
const ENABLED_ENTRY_TYPE = "mcp-adapter";
...
export default function mcpAdapter(pi: ExtensionAPI) {
  ...
  const enabledByFlag = process.argv.includes("--mcp");
  ...
  /**
   * (Re)initialize MCP state for a session. Shuts down any previous state and
   * starts a fresh initialization against the given session context.
   */
  async function initForSession(ctx: ExtensionContext): Promise<void> {
  ...
  /** Tear down MCP state and OAuth for the current session. */
  async function teardownForSession(): Promise<void> {
    ++lifecycleGeneration;
  ...
  function wasEnabledInSession(ctx: ExtensionContext): boolean {
    return ctx.sessionManager.getEntries().some(
      (entry) =>
        entry.type === "custom" &&
        entry.customType === ENABLED_ENTRY_TYPE &&
  ...
  if (enabledByFlag) {
    // Flag mode: activate at load time, (re)initialize on every session start.
    registerSurface();
    pi.on("session_start", async (_event, ctx) => {
      await initForSession(ctx);
    });
    pi.on("session_shutdown", async () => {
      await teardownForSession();
    });
  } else {
    // Command mode: no-op at startup. Activation happens via /enable-mcp, or
    // automatically when a session carrying the enabled marker starts (e.g.
    // after /mcp setup triggers a reload of an enabled session).
    pi.on("session_start", async (_event, ctx) => {
      if (wasEnabledInSession(ctx)) {
        registerSurface();
        await initForSession(ctx);
      } else {
        // Switching away from an enabled session: drop stale MCP state.
        await teardownForSession();
      }
    });
    pi.on("session_shutdown", async () => {
